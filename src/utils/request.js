...
service.interceptors.response.use(
  ...
  async error => {
    ...
    if (error.response && error.response.status === 401) {
      ...
      if (!authorization || !authorization.refresh) {
      ...
      try {
        const res = await axios({
          ...
          timeout: 10000,
          headers: {
            Authorization: `Bearer ${authorization.refresh}`,
          },
          data: { refresh: authorization.refresh },
        })
        ...
        // })
        setToken({
          access: res.data.data.access, // 最新获取的可用 token
          refresh: authorization.refresh, // 还是原来的 refresh_token
        })
        // 把之前失败的用户请求继续发出去
